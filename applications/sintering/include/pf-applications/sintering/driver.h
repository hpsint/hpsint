    ...
    create_grid(InitialMesh initial_mesh, InitialRefine global_refine)
    {
      ...
      if (initial_mesh == InitialMesh::Interface)
      ...
      else
        {
          ...
          n_refinements_remaining = create_mesh_from_divisions(
            tria,
            boundaries.first,
            boundaries.second,
            subdivisions,
            params.geometry_data.periodic,
            params.geometry_data.divisions_data.n_refinements);
        }
        ...
        if (transfer) // update multigrid levels
          {
            ...
            for (unsigned int l = min_level; l <= max_level; ++l)
              mg_sintering_data[l].time_data.copy_all_dt(
                sintering_data.time_data);
