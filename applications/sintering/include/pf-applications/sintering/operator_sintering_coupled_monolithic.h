...
namespace Sintering
{
  ...
  class SinteringOperatorCoupledMonolithic
    : public SinteringOperatorCoupledBase<dim, Number, VectorizedArrayType, T>
  {
  public:
    ...
    SinteringOperatorCoupledMonolithic(
      ...
      : SinteringOperatorCoupledBase<dim, Number, VectorizedArrayType, T>(
          matrix_free,
          constraints,
          free_energy,
          data,
    ...
    post_system_matrix_compute() const override
    {
      ...
      for (const unsigned int index : this->zero_c_constraints_indices)
        for (unsigned int d = 0; d < dim; ++d)
          {
            ...
            const unsigned int matrix_index =
              ...
              this->n_additional_components() + d;

            this->system_matrix.clear_row(matrix_index, 1.0);
          }

      for (unsigned int d = 0; d < dim; ++d)
        for (const unsigned int index : this->displ_constraints_indices[d])
          {
            const auto global_index = partitioner->local_to_global(index);
    ...
    do_pre_vmult(BlockVectorType_ &dst, const BlockVectorType_ &src_in) const
    {
      ...
      this->zero_c_constraints_values.resize(
        this->zero_c_constraints_indices.size());

      for (unsigned int i = 0; i < this->zero_c_constraints_indices.size(); ++i)
        {
          const unsigned int index = this->zero_c_constraints_indices[i];

          for (unsigned int d = 0; d < dim; ++d)
            {
              const unsigned int block_index =
                this->data.n_components() + this->n_additional_components() + d;

              this->zero_c_constraints_values[i][d] =
      ...
      for (unsigned int d = 0; d < dim; ++d)
        this->displ_constraints_values[d].resize(
          this->displ_constraints_indices[d].size());
      ...
      for (unsigned int d = 0; d < dim; ++d)
        for (unsigned int i = 0; i < this->displ_constraints_indices[d].size();
             ++i)
          {
            ...
            this->displ_constraints_values[d][i] =
    ...
    do_post_vmult(BlockVectorType_ &dst, const BlockVectorType_ &src_in) const
    {
      ...
      for (unsigned int i = 0; i < this->zero_c_constraints_indices.size(); ++i)
        {
          const auto &index = this->zero_c_constraints_indices[i];
          const auto &value = this->zero_c_constraints_values[i];

          for (unsigned int d = 0; d < dim; ++d)
            dst
              .block(this->data.n_components() +
                     this->n_additional_components() + d)
      ...
      for (unsigned int i = 0; i < this->zero_c_constraints_indices.size(); ++i)
        {
          ...
          for (unsigned int d = 0; d < dim; ++d)
            {
              const unsigned int matrix_index =
                this->n_components() * index + this->data.n_components() +
                this->n_additional_components() + d;
      ...
      for (unsigned int i = 0; i < this->zero_c_constraints_indices.size(); ++i)
        {
          ...
          for (unsigned int d = 0; d < dim; ++d)
            {
              const unsigned int matrix_index =
                this->n_components() * index + this->n_additional_components() +
