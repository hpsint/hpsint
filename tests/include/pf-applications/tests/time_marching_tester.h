...
namespace Test
{
  ...
  public:
        ...
        post_operation = {}) const
    {
      ...
      // There should be a proper time derivative evaluation here to test
      // implicit schemes too
  ...
  struct MarchingSet
  {
    ...
    MarchingSet(const std::string                     &label_in,
      ...
      , statistics(
          std::make_shared<NonLinearSolvers::NewtonSolverSolverControl>(
  ...
  struct TimeMarchingTester
  {
    TimeMarchingTester()
      : nonlinear_operator(data)
    ...
    test(std::vector<MarchingSet> &time_marchings) const
    {
      ...
      for (auto &marching_set : time_marchings)
        add_zero_entry(marching_set.label);

      auto exact_solution = [](Number t) {
        return std::vector{0.5 * std::exp(-t) +
                             0.5 * (std::sin(t) - std::cos(t)),
                           std::sin(t)};
      };
          ...
          for (auto &marching_set : time_marchings)
            {
              ...
              table.add_value(marching_set.label + " x",
    ...
    SinOperator               nonlinear_operator;
